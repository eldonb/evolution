...
\title{\bf evolution of highly fecund haploid populations \\ probability of
losing fittest type }
...
This code simulates viability selection  in a haploid population characterised
 by high fecundity and sweepstakes reproduction (HFSR).  We  estimate  the
 probability of losing the allelic type with highest fitness from the
 population before the type can reach a given frequency. We exclude mutation. 
...
\be\label{eq:alg}
    w(z) =  \frac{1}{ 1 +  s(z - z_0)^2 }, \quad z \in [0,1].
\ee
  ...
  time (generation) $r$. Define $p_0 := \mathbb{P} \left( Y_r = 0 : Y_r < y \right)$
  as the probability that the fittest allelic type is lost from the population
  \emph{before} reaching frequency $y$.   For comparison with our HFSR
...
ctangle} to generate a {\tt .c} file. The {\tt GNU Scientific Library} is
required.  A compilation to an executable {\tt a.out}  can be obtained with 
\begin{verbatim}
gcc -Wall -Ofast -o a.out file.c -lm -lgsl -lgslcblas
\end{verbatim}
The necessary parameters are defined in section~\ref{sec:main}.   The command,
with {\tt 12345} the random seed, 
\begin{verbatim}
./a.out 12345 out.out
\end{verbatim}
...
1
\end{verbatim}
indicating that in all 10 times the fittest allelic type was lost from the
population before reaching high frequency.  
...
`Mersenne~Twister' random number generator  as
implemented in  GSL.
...
\bd
  \pr{X_i = k} =  C\left( k^{-\alpha}   -  (1+k)^{-\alpha}\right), \quad k \in [\psi], 
\ed 
...
double update_population( int N, double variance, int nalleles,    double s,  double znull,  double epsilon, int * Pop,  int * tXi, int * tempJuve,   double *Z,
double *locuseffects,  double *etimes, size_t * aindex,   gsl_rng * r )
{@#
  ...
  0$ */
   ...
   if( tXi[i] > 0 ){ @#
    ...
    variance) : 0.)   +   locuseffects[ tempJuve[xindex + 1] ]; @#
           @t \newline @>@#
           /* compute fitness value $w = 1/(1 + s(z_i - z_0)^2)$; now
    exponential fitness  $w(z_i) =  \exp\left( -s(z_i - z_0)^2 \right)$  */
             @t \newline @>@#

                w =  gsl_sf_exp( -s*  gsl_pow_2( Z[xindex+1]  -  znull) ); @#
             @q w =  1./( 1. +  (s * gsl_pow_2( Z[xindex+1]  -  znull)) ) ; @>
    ...
    $i$, in ascending order of the associated exponential times; we compute and return the fraction of the null type, the most fit type */
             ...
             @q Zbar = Zbar  +  ( Z[ aindex[i] ] / ( (double)N ) );  @>
              @t \newline @> 
              /* $Zbar$ counts the number of  alleles of the fittest type; type $0$ now the fittest type */
               @t \newline @> 
...
@q printf("runs %d\n", nruns ); @>
 ...
 @t \newline @>
  ...
  @t \newline @>
        ...
        @q  Pop[k] = (int)gsl_rng_uniform_int (r, (unsigned long int)nalleles); @>

        @q Pop[k]  =  (int)(k%nalleles) ; @>
            @t \newline @>
          ...
          @t \newline @>
          /* $X0$ is frequency of  allele with highest  fitness; now allelic type  $0$ with highest fitness */
          @t \newline @>
        ...
        variance) : 0.)  +  leffects[ Pop[k] ] ; @#
         ...
         @t \newline @>
         /* $\varepsilon$ is  the fraction of  the null type - the most fit
         type;  return 0 if most fit type fixes, return 1 if most fit type
         goes extinct \emph{before} reaching $\varepsilon$ in frequency */
           @t \newline @>
...
#define ALPHA 1.0
@t \newline @>
/*If {\tt ALPHA} $(\alpha)$ \eqref{eq:skew} is 0 then the Poisson distribution is assumed with
mean  {\tt BETA} */
@t \newline @>
